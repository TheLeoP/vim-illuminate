...
local function point_in_range(point, range)
  if point.row == range["start"]["line"] and point.col < range["start"]["character"] then return false end
  if point.row == range["end"]["line"] and point.col > range["end"]["character"] then return false end
  return point.row >= range["start"]["line"] and point.row <= range["end"]["line"]
end

local function cursor_in_references(bufnr)
  if not references[bufnr] then return false end
  if vim.api.nvim_win_get_buf(0) ~= bufnr then return false end
  ...
  for _, reference in pairs(references[bufnr]) do
    ...
    if point_in_range({ row = crow, col = ccol }, range) then return true end
  end
  return false
end
...
local function handle_document_highlight(result, bufnr, client_id)
  ...
  if btimer then vim.loop.timer_stop(btimer)     -- vim.loop.close(btimer)
end
  ...
  timers[bufnr] = vim.defer_fn(function()
    ...
    if cursor_in_references(bufnr) then
      ...
      if client then vim.lsp.util.buf_highlight_references(bufnr, result, client.offset_encoding) end
    end
...
local function autocmd(bufnr)
  vim.cmd(
    string.format(
      'autocmd CursorMoved,CursorMovedI <buffer=%d> lua require"illuminate".on_cursor_moved(%d)',
      bufnr,
      bufnr
    )
  )
...
function M.on_attach(client)
  M.stop_buf()
  if client and not client.supports_method "textDocument/documentHighlight" then return end
...
function M.on_cursor_moved(bufnr)
  if not cursor_in_references(bufnr) then vim.lsp.util.buf_clear_references(bufnr) end
  ...
  if vim.lsp.get_clients then
    ...
    for _, client in ipairs(vim.lsp.get_clients { bufnr = bufnr }) do
      if client and client.supports_method "textDocument/documentHighlight" then supported = true end
    end
    -- For older versions
  elseif vim.lsp.for_each_buffer_client then
    supported = false
    vim.lsp.for_each_buffer_client(bufnr, function(client)
      if client.supports_method "textDocument/documentHighlight" then supported = true end
    end)
  end
...
function M.next_reference(opt)
  ...
  if next then
    ...
    if not opt.silent then print("[" .. nexti .. "/" .. #refs .. "]") end
  end
  return next
end
