...
function M.get()
  return (M.large_file_cutoff() == nil or vim.fn.line "$" <= M.large_file_cutoff() or M.large_file_overrides() == nil)
      and config
...
function M.under_cursor(bufnr)
  if M.filetype_override(bufnr)["under_cursor"] ~= nil then return M.filetype_override(bufnr)["under_cursor"] ~= nil end
  return M.get()["under_cursor"]
end

function M.delay(bufnr)
  local delay = M.filetype_override(bufnr)["delay"] or M.get()["delay"] or 17
  if string.sub(vim.api.nvim_get_mode().mode, 1, 1) == "i" then delay = delay + 100 end
  if delay < 17 then return 17 end
  return delay
end
