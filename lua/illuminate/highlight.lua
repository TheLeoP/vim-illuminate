...
function M.buf_highlight_references(buf, references)
  if config.min_count_to_highlight() > #references then return end

  local cursor_pos = util.get_cursor_pos()
  for _, reference in ipairs(references) do
    if config.under_cursor(buf) or not ref.is_pos_in_ref(cursor_pos, reference) then
      M.range(buf, reference[1], reference[2], reference[3])
    end
  end
end

---@param buf integer
---@param start illuminate.Pos
---@param finish illuminate.Pos
---@param kind lsp.DocumentHighlightKind
function M.range(buf, start, finish, kind)
  local start_l, start_col = unpack(start)
  local finish_l, finish_col = unpack(finish)
  start = { buf, start_l + 1, start_col + 1 }
  finish = { buf, finish_l + 1, finish_col + 1 }
  ...
  for _, segment in ipairs(region) do
    ...
    vim.api.nvim_buf_set_extmark(buf, HL_NAMESPACE, start_pos[2] - 1, start_pos[3] - 1, {
...
function M.buf_clear_references(buf)
