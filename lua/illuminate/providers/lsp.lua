...
local function _str_byteindex_enc(line, col, encoding)
  ...
  if vim.fn.has "nvim-0.11" == 1 then return vim.str_byteindx(line, encoding, col, false) end
...
function M.get_references(bufnr)
  if not bufs[bufnr] or not bufs[bufnr][3] then return nil end
...
function M.initiate_request(bufnr, winid)
  ...
  local cancel_fn = vim.lsp.buf_request_all(bufnr, "textDocument/documentHighlight", params, function(client_results)
    if bufs[bufnr][1] ~= id then return end
