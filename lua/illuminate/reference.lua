...
local function buf_sort_references(bufnr)
  ...
  if should_sort then table.sort(get_references(bufnr), ref_before) end
end
...
function M.buf_cursor_in_references(bufnr, cursor_pos)
  if not get_references(bufnr) then return false end

  local i = M.bisect_left(get_references(bufnr), cursor_pos)

  if i > #get_references(bufnr) then return false end
  if not M.is_pos_in_ref(cursor_pos, get_references(bufnr)[i]) then return false end
